...
function formatProduct(p: any): Product {
    ...
    return {
        ...
        priceRange: {
            maxVariantPrice: { amount: p.priceRangeMax.toString(), currencyCode: 'USD' },
            minVariantPrice: { amount: p.priceRangeMin.toString(), currencyCode: 'USD' }
        },
...
function formatCart(cart: any): Cart {
    ...
    const lines = cart.lines?.map((line: any) => {
        ...
        return {
            ...
            cost: {
                totalAmount: { amount: line.costAmount.toString(), currencyCode: 'USD' }
            },
            merchandise: {
...
async function updateCartTotals(cartId: string): Promise<Cart> {
    ...
    const totalQuantity = cartLines.reduce((acc: number, line: any) => acc + line.quantity, 0);
    const subtotalAmount = cartLines.reduce((acc: number, line: any) => acc + Number(line.costAmount), 0);
...
export async function getCollectionProducts({ collection, reverse, sortKey }: { collection: string; reverse?: boolean; sortKey?: string }): Promise<Product[]> {
    ...
    if (sortKey && sortKey !== 'RELEVANCE') {
        orderBy[sortKey === 'CREATED_AT' ? 'createdAt' : sortKey.toLowerCase()] = reverse ? 'desc' : 'asc';
    } else {
