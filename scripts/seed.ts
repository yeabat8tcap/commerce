...
async function main() {
    ...
    for (let i = 1; i <= 10; i++) {
        const title = `Mac Studio with M3 Ultra chip (${i}/10)`;
        const handle = `mac-studio-m3-ultra-${i}`;
        ...
        const product = await prisma.product.create({
            data: {
                ...
                priceRangeMin: 45000.0,
                priceRangeMax: 45000.0,
                ...
                seo: JSON.stringify({ title, description: `${title} ($45000)` }),
